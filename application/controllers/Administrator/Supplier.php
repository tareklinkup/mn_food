...
class Supplier extends CI_Controller {
    ...
    public function getSupplierDue(){
        ...
        $supplierDues = $this->db->query("
            ...
            (select ifnull(sum(mp.total), 0) from tbl_material_purchase mp
                where mp.supplier_id = s.Supplier_SlNo
                and mp.status = 'a') as material_bill,

            (select ifnull(sum(mp.paid), 0) from tbl_material_purchase mp
                where mp.supplier_id = s.Supplier_SlNo
                and mp.status = 'a') as paid_for_material,
            ...
            (select invoicePaid + cashPaid + paid_for_material) as paid,
            
            (select (bill + material_bill + cashReceived) - (paid + returned)) as due
    ...
    public function getSupplierLedger(){
        ...
        $payments = $this->db->query("
            ...
            select
                ...
                concat('Material Purchase - ', mp.invoice_no) as description,
                mp.total as bill,
                mp.paid,
                mp.due,
                0.00 as returned,
    ...
    public function getSupplierPayments(){
        ...
        $payments = $this->db->query("
            ...
            left join tbl_employee as e on e.Employee_SlNo = sp.employee_id
    ...
    public function addSupplierPayment(){
        ...
        try{
            ...
            $supplierInfo = $this->db->query("select * from tbl_supplier where Supplier_SlNo = ?", $paymentObj->SPayment_customerID)->row();

            $message = "{$supplierInfo->Supplier_Name}.\nOur payment amount is {$paymentObj->SPayment_amount}, ref Id {$payment['SPayment_invoice']}";
            $recipient = $supplierInfo->Supplier_Mobile;
