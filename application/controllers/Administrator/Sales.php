...
//    }
    ...
    public function printAllSalesInvoice($userFullName, $customerId, $employeeId, $dateFrom, $dateTo) {
        $branchId = $this->session->userdata("BRANCHid");
        $clauses = "";
        if(isset($dateFrom) && $dateFrom != '' && $dateFrom != 'null' && isset($dateTo) && $dateTo != '' && $dateTo != 'null'){
            $clauses .= " and sm.SaleMaster_SaleDate between '$dateFrom' and '$dateTo'";
        }

        if(isset($userFullName) && $userFullName != '' && $userFullName != 'null'){
            $clauses .= " and sm.AddBy = '".urldecode($userFullName)."'";
        }
        ...
        if(isset($employeeId) && $employeeId != '' && $employeeId != 'null'){
            $clauses .= " and sm.employee_id = '$employeeId'";
        }

        if(isset($customerType) && $customerType != '' && $customerType != 'null'){
            $clauses .= " and c.Customer_Type = '$customerType'";
        }
        ...
        $data['title'] = "Print all invoice";
        $data['sales'] = $sales;
        $data['content'] = $this->load->view('Administrator/sales/print_all_sales_invoice', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
}
