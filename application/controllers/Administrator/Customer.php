...
class Customer extends CI_Controller
{
    ...
    public function getCustomerPayments(){
        ...
        $payments = $this->db->query("
            ...
            left join tbl_employee as e on e.Employee_SlNo = cp.employee_id
    ...
    public function addCustomer()
    {
        ...
        try{
            ...
            $duplicateMobileQuery = $this->db->query("select * from tbl_customer where Customer_Mobile = ?", $customerObj->Customer_Mobile);
    ...
    public function getCustomerWishInvoice(){
        $data = json_decode($this->input->raw_input_stream);
        $invoices = $this->db->query("
            select 
             s.SaleMaster_InvoiceNo	 as invoice_number,
             s.SaleMaster_SlNo as sale_id

            from tbl_salesmaster as s
            where s.Status = 'a'
            and SaleMaster_branchid = ?
            and s.SalseCustomer_IDNo = ?
        ", [$this->session->userdata("BRANCHid"), $data->customerId])->result();

        echo json_encode($invoices);
    }
    public function getInvoiceWishDue(){
        $data = json_decode($this->input->raw_input_stream);
        $due = $this->db->query("
            select 
                *,
                ifnull(((SaleMaster_DueAmount+customer_payment) - customer_receive),0) as invoice_due
            from(
                SELECT
                    ...
                    sum(ifnull(
                        (
                            case 
                                when cp.CPayment_TransactionType ='CP' then cp.CPayment_amount
                            end
                        )
                    ,0)) as customer_payment,
                    sum(ifnull(
                        (
                            case 
                                when cp.CPayment_TransactionType ='CR' then cp.CPayment_amount
                            end
                        )
                    ,0)) as customer_receive
                    
                FROM tbl_salesmaster as sm
                JOIN tbl_customer_payment as cp on cp.sale_id = sm.SaleMaster_SlNo and cp.CPayment_status ='a'
                WHERE sm.Status ='a'
                AND sm.SaleMaster_branchid = ".$this->session->userdata("BRANCHid")."
                AND sm.SaleMaster_SlNo = '$data->saleId'
            ) as t
        ")->row();
