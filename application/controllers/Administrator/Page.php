...
class Page extends CI_Controller {
    ...
    public function company_profile_Update(){
        $inpt = $this->input->post('inpt',true);
        $is_show_due_option = $this->input->post('is_show_due_option',true);
        $id= $this->brunch;
        $fld = 'company_BrunchId';
        $this->load->library('upload');
        $config['upload_path'] = './uploads/company_profile_org/';
        ...
        {   
            $data['print_type'] = $inpt;
            $data['is_show_due_option'] = $is_show_due_option;
    ...
    public function getGodowns(){
        ...
        $clause = "";
        if(isset($data->branchId) && $data->branchId != "" && $data->branchId != null){
            $clause .=" and g.parent_id = $data->branchId";
        }
        ...
        $branches = $this->db->query("
            ...
            b.Brunch_name as parent_branch_name

            from tbl_brunch as g
            left join tbl_brunch as b on b.brunch_id = g.parent_id
            where g.is_godown = true
            and g.status='a'
            $clause
        ")->result();
        echo json_encode($branches);
    ...
    public function addGodown()  {
        ...
        $data['content'] = $this->load->view('Administrator/brunch/add_godown', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function addBranch()  {
        ...
        try{
            ...
            $newBranch = array(
                ...
                'is_godown'=> $branch->is_godown,
                'parent_id'=> $branch->parent_id,
                'status' => 'a'
            );

            $this->db->insert('tbl_brunch', $newBranch);
            $res = ['success'=>true, 'message'=>'Operation successfully'];
        } catch (Exception $ex){
            $res = ['success'=>false, 'message'=>$ex->getMessage()];
        }
