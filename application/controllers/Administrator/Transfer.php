<?php
    class Transfer extends CI_Controller{
        ...
        public function addProductTransfer(){
            ...
            try{
                ...
                $transfer = array(
                    ...
                    'godown_id' => $data->transfer->godown_id,
                ...
                foreach($data->cart as $cartProduct){
                    ...
                    $branch_godown_id = $data->transfer->godown_id == 0 ? $data->transfer->transfer_to : $data->transfer->godown_id;

                    $transferToBranchInventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$cartProduct->product_id, $branch_godown_id])->num_rows();
                    if($transferToBranchInventoryCount == 0){
                        $transferToBranchInventory = array(
                            'product_id' => $cartProduct->product_id,
                            'transfer_to_quantity' => $cartProduct->quantity,
                            'branch_id' => $branch_godown_id
                        );
                        ...
                        $this->db->query("
                            ...
                            where product_id = ?
                            and branch_id = ?
                        ", [$cartProduct->quantity, $cartProduct->product_id, $branch_godown_id]);
                    }
                }
            ...
            try{
                ...
                foreach($oldTransferDetails as $oldDetails) {
                    ...
                    $this->db->query("
                        ...
                        where product_id = ?
                        and branch_id = ?
                    ", [$oldDetails->quantity, $oldDetails->product_id, $oldTransfer->godown_id == 0 ? $oldTransfer->transfer_to : $oldTransfer->godown_id]);
                }
        ...
        public function getReceives(){
            ...
            $branchOrGodownClause = "";
            $godownId = $this->session->userdata('BRANCHid');
            if($this->session->userdata("active_godown")){
                $branchOrGodownClause .= " and tm.godown_id =$godownId";
            }else{
                $branchOrGodownClause .= " and tm.transfer_to =$godownId";
            }
