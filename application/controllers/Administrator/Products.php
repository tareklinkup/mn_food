...
class Products extends CI_Controller
{
    ...
    public function getProductStock()
    {
        ...
        $clauses = "";
        if (isset($inputs->productId) && $inputs->productId != '' && $inputs->productId != null) {
            $clauses .= " and p.Product_SlNo = $inputs->productId";
        }
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(pd.PurchaseDetails_TotalQuantity), 0) 
                        ...
                        JOIN tbl_purchasemaster as pm on pm.PurchaseMaster_SlNo = pd.PurchaseMaster_IDNo
                        where pd.Product_IDNo = p.Product_SlNo
                        and pd.PurchaseDetails_branchID = '$branchId'
                        and pd.Status = 'a' and pm.purchase_type !='pending') as purchased_quantity,
                ...
                (select ifnull(sum(pp.quantity), 0)
                        ...
                        where pp.product_id = p.Product_SlNo and pp.status='a' and pp.branch_id='$branchId') as production_quantity,
    ...
    public function getCurrentStock()
    {
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(pp.quantity), 0)
                        from tbl_production_products pp
                        where pp.status = 'a' and pp.product_id = p.Product_SlNo and pp.status = 'a' and pp.branch_id='$branchId') as production_quantity,
