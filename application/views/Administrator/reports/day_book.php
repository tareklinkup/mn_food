	...
	new Vue({
		...
		methods: {
			...
			getMaterialPurchases() {
				axios.post('/get_material_purchase', this.filter)
					.then(res => {
						let purchases = res.data.purchases.filter(purchase => purchase.paid > 0);
						purchases = _.groupBy(purchases, 'supplier_id');
						purchases = _.toArray(purchases);
						purchases = purchases.map(purchase => {
							purchase[0].totalAmount = purchase.reduce((p, c) => { return p + parseFloat(c.paid) }, 0);
							return purchase[0];
						})
			...
			getEmployeePayments(){
				...
				.then(res => { 
					...
					payments = _.toArray(payments);
					payments = payments.map(payment => {
						payment[0].totalAmount = payment.reduce((p, c) => { return p + parseFloat(c.payment_amount) }, 0);
						return payment[0];
					})
