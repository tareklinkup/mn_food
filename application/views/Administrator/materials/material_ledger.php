	...
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;">
		<form v-on:submit.prevent="getMaterialLedger">
			<div class="form-group">
				...
				<div class="col-sm-2">
					<v-select v-bind:options="allMaterials" v-model="selectedMaterial" label="name"></v-select>
				</div>
			</div>
			...
			<table class="table table-bordered">
				...
				<tbody>
					<tr>
						...
						<td colspan="3"></td>
						<td style="text-align:right;">{{ parseFloat(prevous_qty).toFixed(2) }}</td>
					</tr>
					<tr v-for="row in ledger">
						<td>{{ row.date }}</td>
						<td style="text-align:left;">{{ row.description }}</td>
						<td style="text-align:right;">{{ parseFloat(row.rate).toFixed(2) }}</td>
						<td style="text-align:right;">{{ parseFloat(row.in_qty).toFixed(2) }}</td>
						<td style="text-align:right;">{{ parseFloat(row.out_qty).toFixed(2) }}</td>
		...
		methods: {
			...
			getMaterialLedger() {
				...
				axios.post('/get_material_ledger', data).then(res => {
					...
					this.prevous_qty = res.data.prev_qty;
