		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				<select class="chosen-select form-control" name="Brunch" id="Brunch" data-placeholder="Choose a Brunch...">
					...
					<?php 
					$sql = $this->db->query("SELECT * FROM tbl_brunch where status = 'a' and is_godown = false order by Brunch_name asc");
					$row = $sql->result();
					foreach($row as $row){ ?>
		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				<select class="chosen-select form-control" name="godown" id="godown" data-placeholder="Choose a Godown...">
					...
					<?php 
					$sql = $this->db->query("SELECT * FROM tbl_brunch where status = 'a' and is_godown = true order by Brunch_name asc");
					$row = $sql->result();
					foreach($row as $row){ ?>
					<option value="<?php echo $row->brunch_id; ?>"><?php echo $row->Brunch_name; ?></option>
					<?php } ?>
...
<script type="text/javascript">
    function submit(){
        ...
        var inputdata = 'username='+username+'&rePassword='+rePassword+'&txtFirstName='+txtFirstName+'&user_email='+user_email+'&Brunch='+Brunch+'&type='+type+'&godown='+godown;
