...
<div id="salesRecord">
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: sales.length > 0 ? '' : 'none'}">
		...
		<p class="text-right" v-if="searchType !='category' && searchType !='quantity'">
			<button class="btn btn-xs btn-warning" @click="printAllInvoice()" target="_blank">Print all invoice</button>
		</p>
		<div class="col-md-12">
...
<script>
	...
	new Vue({
		...
		methods: {
			printAllInvoice() {
				let filter = {
					userFullName: this.selectedUser == null || this.selectedUser.FullName == '' ? null : this.selectedUser.FullName,
					customerId: this.selectedCustomer == null || this.selectedCustomer.Customer_SlNo == '' ? null : this.selectedCustomer.Customer_SlNo,
					employeeId: this.selectedEmployee == null || this.selectedEmployee.Employee_SlNo == '' ? null : this.selectedEmployee.Employee_SlNo,
					dateFrom: this.dateFrom,
					dateTo: this.dateTo
				} 
				let url = filter.userFullName+"/"+filter.customerId+"/"+filter.employeeId+"/"+filter.dateFrom+"/"+filter.dateTo;
				window.open('/print_all_sales_invoice/'+url, '_blank');
			},
			onChangeSearchType(){
				this.sales = [];
