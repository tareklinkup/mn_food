...
class Model_Table extends CI_Model
{
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            (
                ...
                where mp.status = 'a'
                and mp.purchase_for = " . $this->session->userdata('BRANCHid') . "
                " . ($date == null ? "" : " and mp.purchase_date < '$date'") . "
            ) as paid_material_purchase,
            (
                select ifnull(sum(sp.SPayment_amount), 0) from tbl_supplier_payment sp
            ...
            (
                select paid_purchase + paid_material_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment
            ) as total_out,
            (
    ...
    public function chalanInvoice()
    {
        ...
        $lastRow = $this->db->query("select * from tbl_chalan order by id desc limit 1");
        if ($lastRow->num_rows() != 0) {
            $newCode = $lastRow->row()->id + 1;
            $zeros = array('0', '00', '000');
            $code = 'CH' . (strlen($newCode) > count($zeros) ? $newCode : $zeros[count($zeros) - strlen($newCode)] . $newCode);
        }
